...
# 3. Use an ifelse() function to display "woof" for animals that are dogs and "meow" for animals that are cats.
...
## Here, the sequence is a list:
elements <- list(1:3, 4:10)
for(element in elements) {
  print(element)
}

## The following for loop counts the values in vector x that are divisible by 2.
x <- c(2,5,3,9,6)
count <- 0
for(val in x) {
  if(val %% 2 == 0) {
...
# The dataset contains concentration and uptake values for plants in Quebec and Mississippi that received a treatment ("chilled" or "nonchilled").
...
### Note that this could be done more quickly using apply(), but that would not teach you about loops.
...
(height <- matrix(c(1:10, 21:30), nrow = 5, ncol = 4))
apply(X = height, MARGIN = 1, FUN = mean)
?apply # To get more information about apply() function.
...
lapply(SimulatedData, mean) # Apply mean to each element of the list.
...
sapply(SimulatedData, mean) # Apply mean to each element of the list.
...
# It is primarily used when the dataset contains different groups (i.e. levels/factors), and we want to apply a function to each of these groups.
head(mtcars)
tapply(mtcars$hp, mtcars$cyl, FUN = mean) # Get the mean hp by cylinder groups.

## Practicing apply() function family
...
# You have realized that your tool for measuring concentration did not work properly. At Mississippi sites, concentrations less than 300 were measured correctly but concentrations >= 300 were overestimated by 20 units.  Use a loop to correct these measurements for all Mississippi sites.
...
plot(x = CO2$conc, y = CO2$uptake, type = "n", cex.lab = 1.4,
     xlab = "CO2 concentration", ylab = "CO2 uptake") # Type "n" tells R to not actually plot the points.
...
# Using what you learned previously on flow control, create a function print_animal
...
# It can be useful to explicitly return() if the routine should end early,
# jump out of the function and return a value.
...
# Using what you learned, write an if statement that tests whether a numeric variable x is zero.
# If not, it assigns cos(x)/x to z, otherwise it assigns 1 to z.
...
## Pre-allocating a data frame : first store rows in a list, then combine them
## all in one go
...
PlotOnStaticMap(mymap, lat=occ_data_df$latitude, lon=occ_data_df$longitude,
                cex=1, pch=16, lwd=3, col = c('red'))
...
# You have realized that your tool for measuring concentration did not work properly. At Mississippi sites, concentrations less than 300 were measured correctly but concentrations >= 300 were overestimated by 20 units.
...
# Correct answer is option 3 (0.12 - 0.20 - 1).
