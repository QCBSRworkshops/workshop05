...
.pull-left[
In the general example below, R would evaluate the expression 5 times by replacing `i` by numbers from 1 to 5.
...
--
.pull-right[
Similarly, in the following example, every instance of `m` is being replaced by each number between `1:10`, until it reaches the last element of the sequence.
...
It is primarily used when the dataset contains different groups <br> (*i*.*e*. levels/factors), and we want to apply a function to each of these groups.
...
For this, you will have to load the $CO_{2}$ dataset using `data(CO2)`, and then use the object `CO2`.
...
# Edit a plot using `for` and `if`
<br>
Plotting uptake vs concentration using `for` loop and `if`.
